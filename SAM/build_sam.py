...
from SAM.modeling.sam_no_prompt import Sam
...
class model_sam(nn.Module):
    def __init__(self,image_size = 512,num_classes=1):
        super().__init__()


        ckpt = "./SAM/sam_vit_b_01ec64.pth"

        self.sam = sam_model_registry["vit_b"](image_size=image_size, num_classes=num_classes, checkpoint=ckpt,
                                               pixel_mean=[0, 0, 0],
                                               pixel_std=[1, 1, 1])


    def forward(self, x, image_size = 256):
        x = self.sam(x, multimask_output=True, image_size=image_size)
        return x["masks"]
...
if __name__ == "__main__":
    ...
    print('-----' * 5)
    data = t.randn(2, 3, 256, 256).to("cuda")

    model_sam = model_sam(image_size=256).to("cuda")

    output = model_sam(data)
